...
function valeurRessourceActuelle(array $personnage, string $clePrincipale, string $cleSecondaire): int {
    if (array_key_exists($clePrincipale, $personnage)) {
        return (int) $personnage[$clePrincipale];
    }

    return (int) ($personnage[$cleSecondaire] ?? 0);
}

function valeurRessourceMaximum(array $personnage, array $clesMaximum, int $valeurDeSecours): int {
    $maximum = 0;

    foreach ($clesMaximum as $cleMaximum) {
        if (array_key_exists($cleMaximum, $personnage)) {
            $maximum = max($maximum, (int) $personnage[$cleMaximum]);
        }
    }

    return max($maximum, $valeurDeSecours);
}
...
function trouverEquipementPourSlot(array $equipements, array $correspondances): ?array {
    foreach ($equipements as $equipement) {
        $texteComparaison = normaliserCleEquipement(
            (string) ($equipement['code_slot'] ?? '') . ' ' . (string) ($equipement['nom_slot'] ?? '')
        );

        foreach ($correspondances as $correspondance) {
            if (str_contains($texteComparaison, normaliserCleEquipement($correspondance))) {
                return $equipement;
            }
        }
    }
...
$vieActuelleAffichee = valeurRessourceActuelle($personnage, 'point_de_vie_actuel', 'vie_actuelle');
$vieMaximumAffichee = valeurRessourceMaximum($personnage, ['point_de_vie_maximum', 'vie_max'], valeurStatPersonnage($personnage, 'point_de_vie'));
$manaActuelAffiche = valeurRessourceActuelle($personnage, 'magie_actuelle', 'mana_actuel');
$manaMaximumAffiche = valeurRessourceMaximum($personnage, ['magie_maximum', 'mana_max'], valeurStatPersonnage($personnage, 'magie'));

$vieActuelleAffichee = max(0, min($vieActuelleAffichee, $vieMaximumAffichee));
$manaActuelAffiche = max(0, min($manaActuelAffiche, $manaMaximumAffiche));
...
function rendreSlotEquipement(?array $equipement, string $texteDefaut, string $classeCss, string $slotCible): void {
    ...
    if ($equipement) {
        ...
        echo '<div class="slot-personnage-contenu slot-personnage-equippe"'
            . ' data-instance-objet-id="' . (int) ($equipement['instance_objet_id'] ?? 0) . '"'
            . ' data-slot-cible="' . htmlspecialchars($slotCible, ENT_QUOTES, 'UTF-8') . '"'
            . ' data-categorie-objet="' . htmlspecialchars((string) ($equipement['categorie_objet'] ?? ''), ENT_QUOTES, 'UTF-8') . '"'
            . ' data-nom-objet="' . htmlspecialchars($nomObjet, ENT_QUOTES, 'UTF-8') . '"'
            . ' data-description-objet="' . htmlspecialchars((string) ($equipement['description_objet'] ?? ''), ENT_QUOTES, 'UTF-8') . '"'
            . ' data-rarete-objet="' . htmlspecialchars((string) ($equipement['rarete_objet'] ?? ''), ENT_QUOTES, 'UTF-8') . '"'
            . ' data-type-objet="' . htmlspecialchars((string) ($equipement['type_objet'] ?? ''), ENT_QUOTES, 'UTF-8') . '"'
            . ' data-poids-objet="' . (int) ($equipement['poids_unitaire'] ?? 0) . '"'
            . ' data-bonus-pv="' . (int) ($equipement['bonus_point_de_vie'] ?? 0) . '"'
            . ' data-bonus-attaque="' . (int) ($equipement['bonus_attaque'] ?? 0) . '"'
            . ' data-bonus-magie="' . (int) ($equipement['bonus_magie'] ?? 0) . '"'
            . ' data-bonus-agilite="' . (int) ($equipement['bonus_agilite'] ?? 0) . '"'
            . ' data-bonus-intelligence="' . (int) ($equipement['bonus_intelligence'] ?? 0) . '"'
            . ' data-bonus-synchronisation="' . (int) ($equipement['bonus_synchronisation_elementaire'] ?? 0) . '"'
            . ' data-bonus-critique="' . (int) ($equipement['bonus_critique'] ?? 0) . '"'
            . ' data-bonus-dexterite="' . (int) ($equipement['bonus_dexterite'] ?? 0) . '"'
            . ' data-bonus-defense="' . (int) ($equipement['bonus_defense'] ?? 0) . '">';
        ...
        echo '<form method="post" class="formulaire-action-equipement formulaire-action-desequiper">'
            . '<input type="hidden" name="action" value="desequiper_objet_equipe">'
            . '<input type="hidden" name="instance_objet_id" value="' . (int) ($equipement['instance_objet_id'] ?? 0) . '">'
    ...
    echo '<form method="post" class="formulaire-equipement-slot">'
        . '<input type="hidden" name="action" value="equiper_objet_slot">'
        . '<input type="hidden" name="instance_objet_id" value="">'
        . '<input type="hidden" name="slot_cible" value="' . htmlspecialchars($slotCible, ENT_QUOTES, 'UTF-8') . '">'
        . '<input type="hidden" name="fenetres_ouvertes" value="inventaire,personnage">'
        . '</form>';
        ...
        <div class="zone-feuille-personnage">
            ...
            <aside class="carte-statistiques">
                ...
                <div class="grille-statistiques-personnage">
                    <div class="ligne-statistique-personnage"><span>PV</span><strong><?= $vieActuelleAffichee ?> / <?= $vieMaximumAffichee ?></strong></div>
                    <div class="ligne-statistique-personnage"><span>Attaque</span><strong><?= valeurStatPersonnage($personnage, 'attaque') ?></strong></div>
                    <div class="ligne-statistique-personnage"><span>Magie</span><strong><?= $manaActuelAffiche ?> / <?= $manaMaximumAffiche ?></strong></div>
