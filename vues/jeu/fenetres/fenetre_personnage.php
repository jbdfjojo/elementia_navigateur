...
function cheminIconeEquipement(?string $iconeObjet): string
{
    ...
    if (str_starts_with($iconeObjet, 'http://') || str_starts_with($iconeObjet, 'https://')) {
        return $iconeObjet;
    }

    if (str_starts_with($iconeObjet, '/')) {
        return $iconeObjet;
    }
...
function rendreSlotEquipement(?array $equipement, string $texteDefaut, string $classeCss, string $slotCible): void
{
    ...
    echo '<div class="slot-personnage ' . htmlspecialchars($classeCss, ENT_QUOTES, 'UTF-8') . ($estOccupe ? ' slot-personnage-occupe' : ' slot-personnage-vide') . '"';
    echo ' data-slot-cible="' . htmlspecialchars($slotCible, ENT_QUOTES, 'UTF-8') . '"';
    echo '>';
    ...
    if ($equipement) {
        ...
        echo ' data-instance-objet-id="' . (int) ($equipement['instance_objet_id'] ?? 0) . '"';
        echo ' data-nom-objet="' . htmlspecialchars($nomObjet, ENT_QUOTES, 'UTF-8') . '"';
        echo ' data-description-objet="' . htmlspecialchars((string) ($equipement['description_objet'] ?? ''), ENT_QUOTES, 'UTF-8') . '"';
        echo ' data-rarete-objet="' . htmlspecialchars((string) ($equipement['rarete_objet'] ?? ''), ENT_QUOTES, 'UTF-8') . '"';
        echo ' data-type-objet="' . htmlspecialchars((string) ($equipement['type_objet'] ?? ''), ENT_QUOTES, 'UTF-8') . '"';
        echo ' data-poids-objet="' . (int) ($equipement['poids_unitaire'] ?? 0) . '"';
        echo ' data-bonus-pv="' . (int) ($equipement['bonus_point_de_vie'] ?? 0) . '"';
        echo ' data-bonus-attaque="' . (int) ($equipement['bonus_attaque'] ?? 0) . '"';
        echo ' data-bonus-magie="' . (int) ($equipement['bonus_magie'] ?? 0) . '"';
        echo ' data-bonus-agilite="' . (int) ($equipement['bonus_agilite'] ?? 0) . '"';
        echo ' data-bonus-intelligence="' . (int) ($equipement['bonus_intelligence'] ?? 0) . '"';
        echo ' data-bonus-synchronisation="' . (int) ($equipement['bonus_synchronisation_elementaire'] ?? 0) . '"';
        echo ' data-bonus-critique="' . (int) ($equipement['bonus_critique'] ?? 0) . '"';
        echo ' data-bonus-dexterite="' . (int) ($equipement['bonus_dexterite'] ?? 0) . '"';
        echo ' data-bonus-defense="' . (int) ($equipement['bonus_defense'] ?? 0) . '"';
        echo '>';
        ...
        echo '<input type="hidden" name="action" value="desequiper_objet_equipe">';
        echo '<input type="hidden" name="instance_objet_id" value="' . (int) ($equipement['instance_objet_id'] ?? 0) . '">';
        echo '</form>';
    } else {
    ...
    echo '<input type="hidden" name="action" value="equiper_objet_slot">';
    echo '<input type="hidden" name="instance_objet_id" value="">';
    echo '<input type="hidden" name="slot_cible" value="' . htmlspecialchars($slotCible, ENT_QUOTES, 'UTF-8') . '">';
    echo '</form>';
...
<div id="fenetre-personnage"
    ...
    <div class="contenu-fenetre-jeu contenu-fenetre-personnage">
        <div class="zone-feuille-personnage">
            ...
            <section class="carte-personnage" aria-label="Équipement du personnage">
            ...
            <aside class="carte-statistiques" aria-label="Statistiques du personnage">
    ...
    <div id="menu-contextuel-equipement" class="menu-contextuel-objet" hidden>
        <button type="button" data-action-menu-equipement="desequiper">Déséquiper</button>
    </div>
</div>